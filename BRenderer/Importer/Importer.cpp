		...
		aiNode* node = node_info.assimp_node;
		{
			if (node->mNumMeshes > 0)
			{
				...
				for (size_t i = 0; i < node->mNumMeshes; i++)
				{
					...
					mesh_component.surfaces[i].vertices.resize(mesh->mNumVertices);
					memcpy(mesh_component.surfaces[i].vertices.data(), &mesh->mVertices, mesh->mNumVertices * sizeof(glm::vec3));

					mesh_component.surfaces[i].indices.resize(mesh->mNumFaces * 3);

					for (uint32_t i = 0, j = 0; i < mesh->mNumFaces; i++, j += 3)
					{
						aiFace face = mesh->mFaces[i];

						mesh_component.surfaces[i].indices[ j ] = face.mIndices[0];
						mesh_component.surfaces[i].indices[j+1] = face.mIndices[1];
						mesh_component.surfaces[i].indices[j+2] = face.mIndices[2];
					}
				}
			}
		}
